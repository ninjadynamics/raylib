...
void DrawModelEx(Model model, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale, Color tint)
{
    Matrix matTransform;
#if defined(PLATFORM_DREAMCAST)
    // DrawModel's origin/unit placement already carries the complete transform
    // in model.transform. Skip only construction of its identity placement;
    // retain the final multiply below (including non-finite/signed-zero rules).
    // Restrict the axis as well: arbitrary non-finite axes need the old path.
    if ((position.x == 0.0f) && (position.y == 0.0f) && (position.z == 0.0f) &&
        (scale.x == 1.0f) && (scale.y == 1.0f) && (scale.z == 1.0f) &&
        (rotationAngle == 0.0f) &&
        (rotationAxis.x == 0.0f) && (rotationAxis.y == 1.0f) && (rotationAxis.z == 0.0f))
    {
        matTransform = MatrixIdentity();
    }
    else
#endif
    {
